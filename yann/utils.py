...
def show_sample_sequence(targets, preds, seq_lengths, recurrent=False):
    ...
    for j in range(6):
        ...
        if recurrent:
            ...
            end_indices = seq_lengths.cumsum(dim=0)
            end_sequence = end_indices[sample_id]

            if sample_id == 0:
                start_sequence = None
            else:
                start_sequence = end_indices[sample_id - 1]

            target = targets[start_sequence:end_sequence]
            pred = preds[start_sequence:end_sequence]

        for i in range(target.size(-1)):
            if i == 0 and j == 0:
                ax.plot(target[:, i], color='C{}'.format(j), label='Target')
                ax.plot(pred[:, i], ':', color='C{}'.format(j), label='Prediction')
            else:
                ax.plot(target[:, i], color='C{}'.format(j), label='_nolegend_')
                ax.plot(pred[:, i], ':', color='C{}'.format(j), label='_nolegend_')
